...
%>>>>>>> 42bd9b058b74b17e4d33570f477bc559277efc63
\pagestyle{empty}
\usepackage{float}
%\usepackage{wrapfig}
\usepackage{babel}
...
\begin{figure}
	...
	\end{center}
	\caption{Diagramme de classe utilisé dans la bibliothèque d'analyse}
	\label{DiagrammeClasseLib}
\end{figure}

\subsection{\color{blue} Details de l'implémentation de la lib}
Comme specifié plus haut, nous avons 3 classes et chacune definie un certain niveau \textcolor{green}{d'abstraction} d'une boucle
\subsubsection{\color{blue} Classe Mihp\_Adress}
  Cette classe représente des objets de type adresse mémoire, à chaque fois au'une itération de la boucle accéde à une case mémoire (qui est généralement une variable), on crée un objet Mihp\_Adress qui va contenir 3 informations essentielles à notre analyse : l'adresse de début de cette case mémoire, le type d'accés qui a été fait sur cette adresse mémoire ainsi que la taille en nombre d'octets de cette case mémoire. \\
  Donc nous aurons pour chaque itération, autant d'objets Mihp\_Adress qu'il y aura d'objets ( variables, constantes ...) manippulés et si il n'y a pas de conditions ou des structures de branchements, nous aurons le méme nombre d'objets dans toutes les itérations.\\
